...
export default Controller.extend({
        ...
        try {
            ...
            if (isNew && post.get('id')) {
                if (!this.leaveEditorTransition) {
                    this.replaceRoute('poster-editor.edit', post);
                }
                return true;
            }
    ...
    willTransition(transition) {
        ...
        let fromNewToEdit = this.get('router.currentRouteName') === 'poster-editor.new'
            && transition.targetName === 'poster-editor.edit'
    ...
    _showSaveNotification(prevStatus, status, delay) {
        ...
        if (status === 'published') {
            type = this.get('post.poster') ? 'Poster' : 'Post';
